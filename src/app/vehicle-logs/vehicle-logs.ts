...
import { Observable, Subject, Subscription, from, BehaviorSubject, combineLatest } from 'rxjs';
...
export class VehicleLogs implements OnInit, OnDestroy {
  ...
  filteredRecords$!: Observable<vehicleLog[]>;
  ...
  ngOnInit(): void {
    ...
    this.filteredRecords$ = combineLatest([
      this.parkingService.parkingRecords$,
      this.searchTerm$,
    ]).pipe(
      map(([records, term]) =>
        records.filter((r) => r.vehicleNumber.toLowerCase().includes(term.toLowerCase()))
      )
    );

    const searchSub = this.userSearch$
      .pipe(
        debounceTime(300),
    ...
    const searchInputSub = this.entryForm
      .get('phoneSearchInput')
    ...
    this.parkingService.searchUsersByPhone(phone).subscribe((users) => {
      if (users.length > 0) {
        ...
        this.entryForm.patchValue({
          userId: phone,
          customerNmae: 'Walk-in User',
        });
        this.userSearchResults = [];
        this.userSearchSuccess = '';
        this.userSearchError = 'No user found. Phone number will be used as User ID.';
      }
    });
  }
  // For search bar
  ...
  onSubmitLogEntry(): void {
    ...
    const customerName = formValue.userId
      ? formValue.customerName
      : `Walk-in (${formValue.phoneSearchInput})`;
